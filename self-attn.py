...
def fused_self_attn_for_SD_small_head_size(q_ref, k_ref, v_ref, use_causal_mask=False,
  ...
  trans_v = nl.ndarray((nl.par_dim(v_seq_tile_size), v_seq_n_tiles, d_head), dtype=pe_in_dt)
  ...
  q_local = nl.ndarray((q_seq_n_tiles, nl.par_dim(d_head_tile_size), q_seq_tile_size), dtype=pe_in_dt)
  ...
  k_local = nl.ndarray((k_seq_n_tiles, nl.par_dim(d_head_tile_size), k_seq_tile_size), dtype=pe_in_dt)
  ...
  for i_q_seq_tile in nl.affine_range(q_seq_n_tiles):  # indent = 2
    ...
    qk_res_buf = nl.ndarray((nl.par_dim(q_seq_tile_size), seqlen), dtype=kernel_dtype)

    neg_max_res = nl.ndarray((nl.par_dim(q_seq_tile_size), k_seq_n_tiles), dtype=kernel_dtype)
    ...
    softmax_res = nl.ndarray((nl.par_dim(q_seq_tile_size), seqlen), dtype=pe_in_dt)
    sum_divisor = nl.ndarray((nl.par_dim(q_seq_tile_size), d_head_tile_size), dtype=kernel_dtype)
